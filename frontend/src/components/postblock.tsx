    ...
    const toggleLike = async () => {
        ...
        try {
            if (isLiked && existingLike) {
                await likeApi.remove(existingLike.like_id);
                setLikes((prev: any[]) => prev.filter((l) => l.like_id !== existingLike.like_id));
            } else {
                const res = await likeApi.create({ user: { user_id: currentUserId }, post: { post_id: post.post_id } });
    ...
    return (
        ...
        >
            ...
            <div className="interact flex space-x-6 text-sm text-gray-300">
                ...
                <div
                    className={`like-count flex items-center gap-1 hover:text-white cursor-pointer transition ${likeLoading ? 'opacity-60 pointer-events-none' : ''}`}
                    onClick={(e) => { e.stopPropagation(); toggleLike(); }}
                >
                    <svg xmlns="http://www.w3.org/2000/svg" className={`h-4 w-4 ${isLiked ? 'text-pink-400' : ''}`} fill={isLiked ? 'currentColor' : 'none'} viewBox="0 0 24 24" stroke="currentColor">
